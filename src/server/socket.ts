...
export function registerSocketHandlers(io: Server) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("send_message", async (payload: SendMessagePayload) => {
      const { sessionId, content, images = [], transcripts = [], webpages = [], memoryEnabled = true } = payload;
      ...
      try {
        await runAgentLoop(sessionId, content, images, socket, controller.signal, appSettings, transcripts, webpages, memoryEnabled);
      } finally {
        activeGenerations.delete(sessionId);
      }
    });
